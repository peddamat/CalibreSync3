...
class FileHelper {
  ...
  static func copyBookCovers(
    covers: [String], at remoteDirectory: URL, to localDirectory: URL, bookCount: Binding<CGFloat>
  ) -> Promise<Bool> {
    return Promise<Bool> { seal in
      ...
      DispatchQueue.global(qos: .userInitiated).async {
        ...
        for coverURL in covers {
          ...
          coordinator.coordinate(readingItemAt: srcFile, options: [], error: &error) {
            (url) -> Void in
            do {
