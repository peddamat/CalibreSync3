...
//    }
    ...
    func beginDownloadingFile(){
        ...
        let coordinator = NSFileCoordinator()
        var error: NSError? = nil
        coordinator.coordinate(readingItemAt: URL(string: self.remoteFilePath)!, options: [], error: &error) { (url2) -> Void in
            
            print("Download started for \(self.localFileURL)")
//            DispatchQueue.global(qos: .userInitiated).async {
                let configuration = URLSessionConfiguration.default
                let operationQueue = OperationQueue()
                let urlSession = URLSession(configuration: configuration, delegate: DownloaderDelegate(bookID: self.bookID, progress: self.$progress, localFileURL: self.localFileURL), delegateQueue: operationQueue)
                NSLog(url2.path)
