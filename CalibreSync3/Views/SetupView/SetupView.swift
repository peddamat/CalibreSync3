...
struct SetupView: View {
  ...
  func saveCalibrePath(_ pickedFolderURL: URL) {
    ...
    getFolder().then { _ in
      ...
      return FileHelper.copyBookCovers(
        covers: bookCoverURLs,
        at: pickedFolderURL,
    ...
    ZStack {
      ...
      VStack(alignment: .leading, spacing: 30) {
        ...
        if self.showFoundCalibreDB {
          Text(
            "Great!  Now we're going to download a local copy of your database.  This may take a few minutes."
          )

          HStack {
            Spacer()
            SimpleProgressBar(
              circleProgress: self.$bookCount, bookTotal: self.$bookTotal, width: 200, height: 20,
              progressColor: Color(red: 0 / 255, green: 212 / 255, blue: 255 / 255),
              staticColor: .gray)
            Spacer()
          }
        }
        ...
        if self.step2 {
          ...
          Text(
            "If you ever need to change your Calibre Library location, you can do so from the Setting menu."
          )
        ...
        if self.showDatabaseError {
          Text(
            "Uh oh, we couldn't find a Calibre database in this location.  Try selecting another folder."
          )
        }
    ...
    GeometryReader { geometry in
      ZStack(alignment: .leading) {
        ...
        Capsule()
          ...
          .frame(
            width: (self.circleProgress / self.bookTotal) * geometry.size.width,
...
struct SimpleProgresBar_Previews: PreviewProvider {
  static var previews: some View {
    SimpleProgressBar(
      circleProgress: .constant(0.05), bookTotal: .constant(0.6), width: 200, height: 10,
      progressColor: .blue, staticColor: .gray)
  }
}
