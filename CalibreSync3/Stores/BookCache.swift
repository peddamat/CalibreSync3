...
class BookCache: ObservableObject {
  ...
  func getCover(forBook: DiskBook) -> URL {
    return URL(
      fileURLWithPath: self.store.localLibraryURL!.path + "/" + forBook.path + "/cover.jpg")
  }

  func getRemoteFile(forBook book: DiskBook, withFormat format: DiskBookFormat) -> URL {
    let tempPath =
      self.store.remoteLibraryPath! + "/" + book.path + "/" + format.name + "."
  ...
  func getLocalFile(forBook book: DiskBook, withFormat format: DiskBookFormat) -> URL {
    let tempPath =
      self.store.localLibraryPath! + "/" + book.path + "/" + format.name + "."
      + format.format.lowercased()
    return URL(fileURLWithPath: tempPath)
  }
