...
class CalibreDB {
  ...
  static func copyDatabase(at remoteDirectory: URL, to localDirectory: URL) -> Promise<URL> {
    return Promise<URL> { seal in
      do {
        ...
        coordinator.coordinate(readingItemAt: remoteDBURL, options: [], error: &error) {
          (url) -> Void in
          try! FileManager.default.copyItem(at: url, to: localDBURL)
        }

        // Make database file writable so we can run migrations
        let attributes: [FileAttributeKey: AnyObject] = [
          FileAttributeKey.posixPermissions: NSNumber(value: 0o666)
        ]
      ...
      DispatchQueue.global(qos: .userInitiated).async {
        do {
          try dbQueue.read { db in
            ...
            let bookCoverURLs = newBooks.compactMap {
              createURL(baseURL: withBaseURL, bookPath: $0.path)
            }
  ...
  func setupNotifications() {
    NotificationCenter.default.addObserver(forName: .downloadComplete, object: nil, queue: .main) {
      (notification) in
