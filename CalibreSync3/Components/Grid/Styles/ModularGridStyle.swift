...
public struct ModularGridStyle: GridStyle {
  ...
  public init(
    columns: Tracks, rows: Tracks, axis: Axis = .vertical, spacing: CGFloat = 8,
  ...
  private func layoutPreferences(
    tracks: Int, spacing: CGFloat, axis: Axis, itemSize: CGSize, preferences: [GridItemPreferences]
  ) -> [GridItemPreferences] {
    ...
    preferences.forEach { preference in
      if let minValue = tracksLengths.min(), let indexMin = tracksLengths.firstIndex(of: minValue) {
        ...
        let width =
          axis == .vertical
          ? itemSizeWidth * CGFloat(indexMin) + CGFloat(indexMin) * spacing
          : tracksLengths[indexMin]
        let height =
          axis == .vertical
          ? tracksLengths[indexMin]
          : itemSizeHeight * CGFloat(indexMin) + CGFloat(indexMin) * spacing
        ...
        newPreferences.append(
          GridItemPreferences(
            id: preference.id,
            bounds: CGRect(
              origin: origin, size: CGSize(width: itemSizeWidth, height: itemSizeHeight))
          )
        )
      }
    }
